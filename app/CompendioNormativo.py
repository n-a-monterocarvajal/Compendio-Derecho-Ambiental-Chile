...
def click_sin_firma(page, out_path: Path) -> bool:
    ...
    try:
        selectors = [
            "button:has-text('SIN FIRMA')",
            "a:has-text('SIN FIRMA')",
            "[role='button']:has-text('SIN FIRMA')",
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Genera un compendio PDF de normas ambientales chilenas.")
    ...
    with sync_playwright() as playwright:
        profile_dir = WORK_DIR / "bcn_perfil_chromium"
        ...
        try:
            for index, source in enumerate(config["fuentes"], start=1):
                ...
                file_stub = source.get("archivo") or safe_name(marker)
                path = TMP_DIR / f"{file_stub}.pdf"
    ...
    merge(parts, out_path, config.get("titulo_compendio", "Compendio de normas ambientales de Chile"))
